...
<?php
    if(!isset($_POST["ver"])) {
		...
		$fichero=fopen('ejercicio025.txt', "a+");
		move_uploaded_file($_FILES['foto']['tmp_name'],"./imagenes_25/".$_FILES['foto']['name']);
		fputcsv($fichero, array($_FILES['foto']['name'], $_POST['nombre'],$_POST['comentario']),",",'"');
		rewind($fichero);
		fclose($fichero);
		}
        ?>
	...
	elseif (isset ($_POST['ver'])){
		...
		?> <table>
		  ...
		  <tbody>
		<?
				$archivos = scandir("./imagenes_25");
				while (($array = fgetcsv($fichero,filesize("ejercicio025.txt"),",",'"')) != false){
					?><tr><?
						foreach ($array as $elemento){
							if (in_array($elemento,$archivos)){
							?><td><img src="./imagenes_25/<?=$elemento;?>"/></td>
							<?
							}
							else {
								?><td><?=$elemento;?></td><?
